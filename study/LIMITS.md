# KILL-SWITCH-REFERENCE-001 — LIMITS
...
4. **Owner compromise.** Whoever holds the owner credential can STOP —
   and can also advance epochs. The reference authenticates the role; it
   does not protect the role's key. Labeled HMAC stand-in throughout:
   the tested property is revocation observation and commit ordering,
   not signature strength.
...
lock is fcntl on one filesystem, not a distributed protocol. Twenty race
trials expose obvious race defects; they are not a safety proof.
