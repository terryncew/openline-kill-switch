# KILL-SWITCH-REFERENCE-001 — CLAIM
...
- 7/7 STOPPED under the frozen appraisal rules (attempt + REFUSE +
  independently observed absence, per path).
- P6 closed without changing its semantics: R2's local cache was
  genuinely stale (epoch 0/ACTIVE) and admitted locally; the
  authoritative final gate refused.
...
- STOP/finalize ordering unambiguous: one lock, monotonic sequence,
  commit_seq 1 < stop_effective_seq 2 < all post-stop attempts; 20 race
  trials, 0 forbidden orderings.
- Restart and fail-closed checks hold; replays create no post-stop
  effect; recovery requires deliberate owner action.
